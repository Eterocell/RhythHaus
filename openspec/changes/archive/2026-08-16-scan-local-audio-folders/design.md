...
## Ora-19 Amendment: Single Mutation Publication and Non-Compose Orchestration

Mutation helpers SHALL remove the redundant `updateLibrary`, `updatePlaylists`, `updateError`, and `loadPlaylists` callback surfaces. Each helper SHALL expose one authoritative publication callback, `suspend (LibraryMutationPublication) -> Unit`. The helper SHALL load playlist state internally and publish one combined library-plus-playlist publication; callers SHALL not assemble or publish those parts independently.

The production boundary SHALL include a non-Compose internal App library-mutation orchestrator seam, preferably documented as `shared/src/commonMain/kotlin/com/eterocell/rhythhaus/AppLibraryOrchestrator.kt`. The orchestrator owns operation admission, exception conversion, `complete(token)` in `finally`, and the sole `coordinator.publishIfCurrent` boundary. `App` retains exactly one coordinator and one orchestrator, and Compose/UI code remains a thin delegate to that seam.

The RED/GREEN tests SHALL cover cancellation-await-before-write, repeated mutation admission, stale combined publication, accepted reconciliation/playlist refresh, rejection error-only preservation, and failure completion. Production-boundary acceptance means JVM tests exercise the real `LibraryHomeContent` manager surface plus the non-Compose App seam. Full App Compose/Koin/navigation mounting, real platform picker cancellation, and device runtime are explicit deferrals. Picker cancellation remains platform-asymmetric: JVM reports `Unavailable`, while Android reports no callback. Common tests SHALL assert the no-state-mutation contract for picker cancellation and SHALL not invent a `Cancelled` result for that path.

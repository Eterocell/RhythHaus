...
Review repair: final review identified three defects and they were corrected in `515c8d72` and `7918a159`. Failed-state retry/skip now admit only from the exact captured Error state; surviving Error reconciliation retains its engine generation and recovery surface; Loading/Buffering reconciliation re-enters the lazy selection path without synchronously waiting behind a non-cancellable old engine load; short Split Now Playing layouts scroll only while recovery is visible so recovery and transport controls remain reachable. Focused regressions passed for Error preservation, non-cancellable rebind, failed-track recovery, and 600x400 Split semantics.
Follow-up review of both repairs found no remaining patch-anchored defect and returned Ready. Final focused recovery selectors passed again after formatting; final standalone Spotless, Detekt, architecture, strict OpenSpec, and diff checks passed.
Commits: `86d32880`, `6c088e80`, `1376b8f2`, `cc4ccc2e`, `b6d495be`, `52244a41`, `5d6d0401`, `515c8d72`, `7918a159`.

## Closeout - 2026-09-10 playback failure recovery

Route: openspec+superpowers
Owner: implementation and lifecycle closeout
Input: completed `playback-failure-recovery` implementation, clean whole-branch review, and the remaining real-engine acceptance task.
Output: A permanent JVM/macOS integration regression now drives the production `MacOSNativePlaybackEngine` through a real `PlaybackController` using disposable valid WAV files. It proves initial native playback, selected-file invalidation to structured `MissingFile`, retry of the same occurrence under a new generation, successful non-wrapping skip to a playable successor, no wrap from a failed tail occurrence, removal of exactly one failed occurrence while a duplicate track occurrence survives, successor autoplay, failure clearance, and preservation of the surviving file. Independent review initially found missing duplicate-occurrence, tail/no-wrap, and retry-occurrence assertions; commit `6584be88` addresses all three and the scoped re-review returned correct with no new Critical or Important findings.
Verification:
- `./gradlew :core:playback:jvmTest --tests 'com.eterocell.rhythhaus.PlaybackControllerTest' --rerun-tasks --configuration-cache`: pass, `BUILD SUCCESSFUL`, 20 tasks executed.
- `./gradlew :core:playback:jvmTest --tests 'com.eterocell.rhythhaus.JvmPlaybackEngineTest.nativeMacControllerRecoversWhenPlayingFileBecomesUnavailable' --rerun-tasks --configuration-cache`: pass after final formatting, `BUILD SUCCESSFUL`, 20 tasks executed.
- `./gradlew :core:playback:jvmTest :core:playback:testAndroidHostTest :feature:nowplaying:jvmTest --rerun-tasks --configuration-cache`: pass, `BUILD SUCCESSFUL`, 80 tasks executed.
- `./gradlew :core:playback:compileTestKotlinIosSimulatorArm64 --rerun-tasks --configuration-cache`: pass, `BUILD SUCCESSFUL`, 28 tasks executed.
- `./gradlew :core:playback:jvmTest :core:playback:testAndroidHostTest :core:playback:iosSimulatorArm64Test :feature:nowplaying:jvmTest --rerun-tasks --configuration-cache`: JVM, Android host, and Now Playing tasks completed; the aggregate fails only when Xcode rejects iOS simulator execution because it does not support simulator tests for `ios_simulator_arm64`.
- `./gradlew spotlessApply --configuration-cache`, standalone `spotlessCheck`, standalone `detekt`, and standalone `architectureCheck`: pass.
- `openspec validate playback-failure-recovery --strict` passed before synchronization; after synchronization, `openspec validate --specs` reports 12 passed and 0 failed, and archived-artifact validation remains clean. `git diff --check` passes.
- `./init.sh`: fails in `:shared:jvmTest` at the pre-existing `LibraryPlaybackSelectionTest.differentSelectionPreservesRepeatAndShuffleModes` timeout; the same selector was previously reproduced on `main`. Desktop compilation and Android assembly completed before that failure; no iOS leg ran after the Shared JVM gate failed.
Acceptance limits: No Android device is connected. `EteroiDevClVier` is physically connected, but this harness cannot operate its UI or invalidate a disposable selected file without injecting data into the user's persisted library; that iOS flow is unverified rather than treated as passed. The macOS acceptance uses a valid silent WAV and proves native engine state, not audible output quality.
Next owner: release acceptance on a controllable Android target and iOS runtime, plus independent remediation of the baseline Shared selection-test timeout.

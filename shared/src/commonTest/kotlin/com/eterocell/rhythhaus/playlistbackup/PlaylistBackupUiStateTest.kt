...
class PlaylistBackupUiStateTest {
    ...
    fun previewDismissalAndPanelCancellationAreSilentAndNeverCreateResultState() {
        ...
        val dismissed =
            reducePlaylistBackupUiState(
                preview, PlaylistBackupUiAction.DismissPreview)
        val cancelled =
            reducePlaylistBackupUiState(
                PlaylistBackupUiState(
    ...
    fun distinctRecoverableFailuresRemainDistinct() {
        ...
        assertEquals(
            ...
            failures.map { failure ->
                reducePlaylistBackupUiState(
                        PlaylistBackupUiState(
                            operation = PlaylistBackupOperation.Planning),
    ...
    fun exportBuildsAndFullyDecodesBytesBeforeRequestingSave() = runBlocking {
        ...
        assertIs<PlaylistBackupDecodeResult.Success>(
            PlaylistBackupCodec.decode(ready.bytes))
        Unit
    }
    ...
    fun exportValidationFailureDoesNotProduceSaveableBytes() = runBlocking {
        val result =
            preparePlaylistBackupExport(
                ...
                dispatcher = Dispatchers.Unconfined,
                validate = {
                    PlaylistBackupDecodeResult.Invalid(
    ...
    @Test
    fun importDecodesAndPlansAgainstCurrentTracksWithoutMutation() =
        runBlocking {
            var mutations = 0
    ...
    fun everyValidationErrorMapsToItsExactRecoveryClass() {
        val expected =
            mapOf(
                PlaylistBackupValidationError.INPUT_TOO_LARGE to
                    PlaylistBackupUiError.Oversized,
                PlaylistBackupValidationError.MALFORMED_UTF8 to
                    PlaylistBackupUiError.Malformed,
                PlaylistBackupValidationError.MALFORMED_JSON to
                    PlaylistBackupUiError.Malformed,
                PlaylistBackupValidationError.DUPLICATE_FIELD to
                    PlaylistBackupUiError.Malformed,
                PlaylistBackupValidationError.UNKNOWN_FIELD to
                    PlaylistBackupUiError.Malformed,
                PlaylistBackupValidationError.MISSING_FIELD to
                    PlaylistBackupUiError.Malformed,
                PlaylistBackupValidationError.UNSUPPORTED_FORMAT to
                    PlaylistBackupUiError.UnsupportedVersion,
                PlaylistBackupValidationError.UNSUPPORTED_VERSION to
                    PlaylistBackupUiError.UnsupportedVersion,
                PlaylistBackupValidationError.INVALID_CHECKSUM to
                    PlaylistBackupUiError.Checksum,
                PlaylistBackupValidationError.INVALID_INTEGER to
                    PlaylistBackupUiError.Malformed,
                PlaylistBackupValidationError.NUMERIC_OVERFLOW to
                    PlaylistBackupUiError.Malformed,
                PlaylistBackupValidationError.TRAILING_CONTENT to
                    PlaylistBackupUiError.Malformed,
                PlaylistBackupValidationError.NON_CANONICAL_JSON to
                    PlaylistBackupUiError.Malformed,
                PlaylistBackupValidationError.PLAYLIST_LIMIT_EXCEEDED to
                    PlaylistBackupUiError.InvalidData,
                PlaylistBackupValidationError.PLAYLIST_ENTRY_LIMIT_EXCEEDED to
                    PlaylistBackupUiError.InvalidData,
                PlaylistBackupValidationError.TOTAL_ENTRY_LIMIT_EXCEEDED to
                    PlaylistBackupUiError.InvalidData,
                PlaylistBackupValidationError.STRING_LIMIT_EXCEEDED to
                    PlaylistBackupUiError.InvalidData,
                PlaylistBackupValidationError.BLANK_PLAYLIST_NAME to
                    PlaylistBackupUiError.InvalidData,
                PlaylistBackupValidationError.INVALID_DURATION to
                    PlaylistBackupUiError.InvalidData,
            )

        assertEquals(
            PlaylistBackupValidationError.entries.toSet(), expected.keys)
        PlaylistBackupValidationError.entries.forEach { error ->
            assertEquals(
                expected.getValue(error),
                playlistBackupUiError(error),
                error.name)
        }
    }

    @Test
    fun staleConfirmationRejectsBeforeRepositoryCallAndRetainsPreview() =
        runBlocking {
            ...
            assertEquals(
                PlaylistBackupUiError.StalePreview, outcome.state.error)
            assertSame(state.preview, outcome.state.preview)
        }

    @Test
    fun confirmationConvertsAllEligibleRowsToOneOrderedMutationAndRefreshesOnce() =
        runBlocking {
            val plan =
                plan(
                    restorable = 3,
    ...
    @Test
    fun serializedConfirmationUsesOneOwnerCallThatReturnsTheRefreshedSnapshot() =
        runBlocking {
    ...
    @Test
    fun serializedStaleAndFailureOutcomesRetainPreviewAndConfirmedSnapshot() =
        runBlocking {
            ...
            val failed =
                confirmPlaylistBackupImportSerialized(
                    ...
                    lastConfirmedSnapshot = confirmed,
                    mutateAndRefresh = {
                        PlaylistImportOwnerResult.Failure(
            ...
            assertEquals(
                PlaylistBackupUiError.RepositoryFailed, failed.state.error)
        }

    @Test
    fun repositoryFailureRetainsPreviewAndLastConfirmedSnapshotWithoutRefresh() =
        runBlocking {
            val preview = PlaylistBackupPreview(plan(restorable = 1))
            val confirmedSnapshot = snapshot()
            var ownerCalls = 0
            ...
            assertSame(confirmedSnapshot, outcome.confirmedSnapshot)
            assertEquals(
                PlaylistBackupUiError.RepositoryFailed, outcome.state.error)
            assertEquals(1, ownerCalls)
            ...
            runBlocking {
                confirmPlaylistBackupImportSerialized(
                    state =
                        PlaylistBackupUiState(
                            preview =
                                PlaylistBackupPreview(plan(restorable = 1))),
    ...
    fun cancellationCleanupReturnsIdleAndRetainsPreviewWithoutError() {
        ...
        val cleaned =
            reducePlaylistBackupUiState(
                busy, PlaylistBackupUiAction.OperationCancelled)
    ...
    private fun plan(
        ...
        playlists: List<PlaylistImportPlaylist> =
            if (restorable == 0) {
                ...
                listOf(
                    PlaylistImportPlaylist(
                        0, "Mix", List(restorable) { "track-$it" }),
                )
            },
    ) =
        PlaylistImportPlan(
            ...
            totals =
                PlaylistImportTotals(
                    ...
                    entries =
                        PlaylistImportCounts(restorable, unmatched, ambiguous),
                ),
            issues = emptyList(),
        )
    ...
    private fun track(durationMillis: Long? = 100_000) =
        LibraryTrack(
            id = "track",
            sourceId = "source",

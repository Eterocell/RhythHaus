...
class PlaybackSessionCoordinatorTest {
    @Test
    fun reconcileAppliedEstablishesRevisionWatermarkBeforeDelayedMutationCheckpoint() =
        runBlocking {
            ...
            val gatedController =
                GatedCheckpointSessionController(playbackController)
            val store = RecordingStore()
            val scope = detachedScope(coroutineContext)
            val coordinator =
                PlaybackSessionCoordinator(gatedController, store, scope)
            coordinator.restoreOnce(emptyList())
            val tracks =
                playableTracks(
                    "current-track", "removed-track", "survivor-track")
            ...
            val mutation =
                async(Dispatchers.Default) {
                    playbackController.removeUpcoming("removed-a")
                }
            mutationCommitted.await()
            val reconcile = async {
                coordinator.reconcile(libraryTracks("current-track"))
            }
            ...
            assertEquals(
                PlaybackSessionReconcileResult.Applied, reconcile.await())
            assertEquals(
                listOf(SessionQueueEntry("current-a", "current-track")),
                store.saved.last().queue)
    ...
    @Test
    fun normalizedRestoreEstablishesRevisionWatermarkBeforeOlderCheckpoint() =
        runBlocking {
            ...
            val scope = detachedScope(coroutineContext)
            val coordinator =
                PlaybackSessionCoordinator(controller, store, scope)

            coordinator.restoreOnce(playableTracks("persisted", "normalized"))
            assertEquals(normalized, store.saved.last())
            coordinator.accept(
                PlaybackCheckpoint.Immediate(snapshot("older"), revision = 0L))
            coordinator.flush()
    ...
    fun newerRepeatModeSurvivesDelayedMutationCheckpoint() = runBlocking {
        assertNewerSnapshotSurvivesDelayedMutation(
            applyNewerState = { controller, _, _ ->
                controller.setRepeatMode(RepeatMode.RepeatOne)
            },
            assertDurable = { snapshot ->
                assertEquals(RepeatMode.RepeatOne, snapshot.repeatMode)
            },
        )
    }
    ...
    fun newerShuffleModeSurvivesDelayedMutationCheckpoint() = runBlocking {
        assertNewerSnapshotSurvivesDelayedMutation(
            ...
            assertDurable = { snapshot ->
                assertEquals(ShuffleMode.Off, snapshot.shuffleMode)
            },
        )
    }
    ...
    fun newerSeekPositionSurvivesDelayedMutationCheckpoint() = runBlocking {
        assertNewerSnapshotSurvivesDelayedMutation(
            ...
            assertDurable = { snapshot ->
                assertEquals(700L, snapshot.positionMillis)
            },
        )
    }
        ...
        assertNewerSnapshotSurvivesDelayedMutation(
            prepare = { controller, engine ->
                engine.listener?.onPlaybackStatus(
                    engine.activeGenerationForTest(), PlaybackStatus.Playing)
            },
            applyNewerState = { _, engine, _ ->
                engine.listener?.onPlaybackProgress(
                    engine.activeGenerationForTest(), 1_100L, 2_000L)
            },
            assertDurable = { snapshot ->
                assertEquals(1_100L, snapshot.positionMillis)
            },
        )
    }

    @Test
    fun delayedMutationShuffleCannotOverwriteNewerQueueShuffleOrder() =
        runBlocking {
            val mutationCommitted = CompletableDeferred<Unit>()
                ...
                PlaybackController(
                    ...
                    shuffleOrderFactory = { ids, currentId ->
                        when {
                            ...
                            currentId == "current-b" ->
                                listOf(
                                    "current-b", "upcoming-b-2", "upcoming-b-1")
            ...
            assertEquals(
                "upcoming-b-2", controller.state.value.currentOccurrenceId)
        }

    @Test
    fun nullRevisionCannotRegressDurableStateAfterRevisedCheckpoint() =
        runBlocking {
            ...
            coordinator.accept(
                PlaybackCheckpoint.Immediate(
                    snapshot("revised"), revision = 1L))
            coordinator.accept(
                PlaybackCheckpoint.Immediate(snapshot("legacy-null")))
    ...
    @Test
    fun delayedMutationCheckpointCannotRegressNewerReplacementSnapshot() =
        runBlocking {
            val mutationCommitted = CompletableDeferred<Unit>()
            val allowMutationCheckpoint = CompletableDeferred<Unit>()
            val controller =
            ...
            val tracks =
                playableTracks(
                    "current-track", "upcoming-track", "replacement-track")
            controller.setOccurrenceQueue(
                listOf(
                    QueueOccurrence("current-a", tracks[0]),
                    QueueOccurrence("upcoming-a-1", tracks[1]),
            ...
            controller.setOccurrenceQueue(
                listOf(replacement), selectedOccurrenceId = replacement.id)
            allowMutationCheckpoint.complete(Unit)
            assertEquals(QueueMutationResult.Applied, mutation.await())
            coordinator.flush()

            val replacementSnapshot = controller.sessionSnapshot()
            val obsoleteMutationQueue =
                listOf(
    ...
    @Test
    fun acceptedUpcomingMutationPersistsCompleteOccurrenceSnapshotThroughCoordinator() =
        runBlocking {
            ...
            assertEquals(
                ...
                controller.removeUpcoming("entry-duplicate-1"))
            coordinator.flush()
            ...
            assertEquals(
                "entry-current", store.saved.single().currentOccurrenceId)
            scope.cancel()
        }

    @Test
    fun terminalCompletionFlushPersistsExactNonWholeSecondDurationWithoutProgress() =
        runBlocking {
            val engine = FakePlaybackEngine()
            ...
            coordinator.restoreOnce(emptyList())
            val track =
                playableTracks("terminal")
                    .single()
                    .copy(durationMillis = 1_234L)
    ...
    @Test
    fun flushDuringFirstRestoreWaitsForCollectorReadinessThenCompletes() =
        runBlocking {
            val controller =
                RecordingSessionController(
                    blockRestore = true, failFenceWhenInactive = true)
            val store = RecordingStore()
            val scope = detachedScope(coroutineContext)
            val coordinator =
                PlaybackSessionCoordinator(controller, store, scope)
    ...
    @Test
    fun flushDuringThrowingReadWaitsForFailedSafeCollectorThenCompletes() =
        runBlocking {
            val controller =
                RecordingSessionController(failFenceWhenInactive = true)
            ...
            assertTrue(flush.await().isSuccess)
            assertEquals(
                PlaybackSessionPhase.FailedSafe, coordinator.phase.value)
            assertEquals(1, controller.collectionCount)
            scope.cancel()
        }

    @Test
    fun preRestoreReconcileSaveFailureStillStartsOneCollectorOnRestore() =
        runBlocking {
            val controller =
                RecordingSessionController(failFenceWhenInactive = true)
    ...
    @Test
    fun cancellationWhileFlushWaitsForCollectorReadinessFailsWithoutHanging() =
        runBlocking {
            val controller =
                RecordingSessionController(
                    blockRestore = true, failFenceWhenInactive = true)
            ...
            val scope =
                CoroutineScope(coroutineContext.minusKey(Job) + processJob)
            val coordinator =
                PlaybackSessionCoordinator(controller, RecordingStore(), scope)
            val restore = async {
                runCatching { coordinator.restoreOnce(emptyList()) }
            }
            controller.restoreStarted.await()
            val flush = async { runCatching { coordinator.flush() } }
            kotlinx.coroutines.yield()
    ...
    fun flushWaitsForCollectorFenceBeforeEnqueueingItsBarrier() = runBlocking {
        val controller =
            RecordingSessionController(blockCheckpointCollection = true)
        val store = RecordingStore()
        val scope = detachedScope(coroutineContext)
    ...
    @Test
    fun controlledCheckpointFenceOrderingIsStableAcrossRepeatedFlushes() =
        runBlocking {
            repeat(25) { index ->
                val controller =
                    RecordingSessionController(blockCheckpointCollection = true)
    ...
    @Test
    fun processCancellationCompletesQueuedAndFutureCallersWithoutOrphanedAwait() =
        runBlocking {
            val controller = RecordingSessionController(blockRestore = true)
            val store = RecordingStore()
            val processJob = SupervisorJob()
            ...
            controller.restoreStarted.await()
            val reconcile = async {
                runCatching { coordinator.reconcile(libraryTracks("queued")) }
            }
            val flush = async { runCatching { coordinator.flush() } }
    ...
    @Test
    fun checkpointCollectorFailureTerminatesCoordinatorAndRejectsFutureCommands() =
        runBlocking {
            val controller =
                RecordingSessionController(failCheckpointCollection = true)
            val scope = detachedScope(coroutineContext)
            val coordinator =
                PlaybackSessionCoordinator(controller, RecordingStore(), scope)
            coordinator.restoreOnce(emptyList())
    ...
    @Test
    fun throwingControllerRestoreAttemptsEmptyFallbackAndCompletesFailedSafeCallers() =
        runBlocking {
            val controller =
                RecordingSessionController(throwFirstRestore = true)
            val store = RecordingStore(initial = snapshot("persisted"))
            val scope = detachedScope(coroutineContext)
            val coordinator =
                PlaybackSessionCoordinator(controller, store, scope)
            ...
            withTimeout(5_000) {
                coordinator.restoreOnce(playableTracks("persisted"))
            }

            assertEquals(
                listOf(snapshot("persisted"), PlaybackSessionSnapshot()),
                controller.restoredSnapshots)
            ...
            assertEquals(
                PlaybackSessionReconcileResult.FailedSafeApplied,
                withTimeout(5_000) {
                    coordinator.reconcile(libraryTracks("future"))
                },
            )
    ...
    @Test
    fun restoreDisablesCommandsBeforeReadAndRestoreThenSavesNormalizedStateBeforeCollecting() =
        runBlocking {
    ...
    @Test
    fun reconcileQueuedDuringRestoreRunsAfterRestoreBeforeCallerCompletes() =
        runBlocking {
            val events = mutableListOf<String>()
            val controller =
                RecordingSessionController(events = events, blockRestore = true)
            val scope = detachedScope(coroutineContext)
            val coordinator =
                PlaybackSessionCoordinator(
                    controller, RecordingStore(events = events), scope)
            val restore = async {
                coordinator.restoreOnce(playableTracks("one"))
            }
            controller.restoreStarted.await()

            val reconcile = async {
                coordinator.reconcile(libraryTracks("one"))
            }
            ...
            assertEquals(
                PlaybackSessionReconcileResult.Applied, reconcile.await())
            assertTrue(
                events.indexOf("restore:empty") <
                    events.indexOf("reconcile:one"))
            assertTrue(
                events.indexOf("commands:true") <
                    events.indexOf("reconcile:one"))
            scope.cancel()
        }

    @Test
    fun adjacentCheckpointsCollapseToNewestCompleteSnapshotOnly() =
        runBlocking {
            val controller = RecordingSessionController(blockRestore = true)
            val store = RecordingStore()
            val scope = detachedScope(coroutineContext)
            ...
            assertEquals(
                listOf(PlaybackSessionSnapshot(), snapshot("three")),
                store.saved)
            scope.cancel()
        }
    ...
    fun restoreReconcileAndFlushAreStrictCheckpointBarriers() = runBlocking {
        ...
        coordinator.accept(
            PlaybackCheckpoint.Immediate(snapshot("before-flush")))
        store.blockedSaveStarted.await()
        val flush = async { coordinator.flush() }
        coordinator.accept(
            PlaybackCheckpoint.Immediate(snapshot("after-flush")))
        assertFalse(flush.isCompleted)
        store.allowBlockedSave.complete(Unit)
        flush.await()
        assertEquals(
            listOf(snapshot("before-flush"), snapshot("after-flush")),
            store.saved)

        controller.snapshot = snapshot("reconciled")
        coordinator.accept(
            PlaybackCheckpoint.Immediate(snapshot("before-reconcile")))
        assertEquals(
            PlaybackSessionReconcileResult.Applied,
            coordinator.reconcile(libraryTracks("reconciled")))
        coordinator.accept(
            PlaybackCheckpoint.Immediate(snapshot("after-reconcile")))
        coordinator.flush()
    ...
    @Test
    fun saveFailureCompletesFlushAndFutureReconcileAndRestoreSafely() =
        runBlocking {
            val controller = RecordingSessionController()
    ...
    @Test
    fun replacementLoadFailureEntersFailedSafeWithoutOverwritingDurableSnapshot() =
        runBlocking {
            ...
            val controller =
                RecordingSessionController(
                    ...
                    reconcileFailure =
                        IllegalStateException("replacement load failed"),
                )
    ...
    @Test
    fun throwingReadAppliesEmptyPausedBoundaryReenablesCommandsAndCompletesRestore() =
        runBlocking {
            val controller =
                RecordingSessionController(initialSnapshot = snapshot("old"))
            val store =
                RecordingStore(
            ...
            assertEquals(
                listOf(PlaybackSessionSnapshot()), controller.restoredSnapshots)
            assertEquals(PlaybackSessionSnapshot(), controller.snapshot)
            assertTrue(controller.commandsEnabled)
            assertEquals(
                PlaybackSessionPhase.FailedSafe, coordinator.phase.value)
    ...
    @Test
    fun restoreLoadFailureSavesControllerNormalizedEmptyPausedState() =
        runBlocking {
            val controller =
                RecordingSessionController(
    ...
    @Test
    fun failedSafeIsProcessLifetimeAndFutureCheckpointsNeverPersist() =
        runBlocking {
            val controller = RecordingSessionController()
            val store = RecordingStore(failSaveNumber = 2)
            val scope = detachedScope(coroutineContext)
            ...
            coordinator.restoreOnce(emptyList())
            coordinator.accept(
                PlaybackCheckpoint.Immediate(snapshot("failure")))
            coordinator.flush()

            repeat(3) { index ->
                coordinator.accept(
                    PlaybackCheckpoint.Immediate(snapshot("ignored-$index")))
                coordinator.flush()
                assertEquals(
                    PlaybackSessionReconcileResult.FailedSafeApplied,
    ...
    @Test
    fun repeatedRestoreDoesNotCreateDuplicateCheckpointCollectors() =
        runBlocking {
            val controller = RecordingSessionController()
            val store = RecordingStore()
            val scope = detachedScope(coroutineContext)
    ...
    private suspend fun CoroutineScope
        ...
        applyNewerState:
            (
                PlaybackController,
                FakePlaybackEngine,
                PlaybackSessionCoordinator) -> Unit,
...
private class GatedCheckpointSessionController(
    ...
    override suspend fun reconcileSession(
        tracks: List<PlayableTrack>
    ): RevisionedPlaybackSessionSnapshot = delegate.reconcileSession(tracks)
    ...
    override fun setCommandsEnabled(enabled: Boolean) =
        delegate.setCommandsEnabled(enabled)
}
...
private class RecordingSessionController(
    ...
    private val checkpointChannel =
        Channel<PlaybackCheckpoint>(Channel.UNLIMITED)
    ...
    override suspend fun restoreSession(
        ...
        restoreAttempts++
        if (throwFirstRestore && restoreAttempts == 1)
    ...
    override suspend fun awaitCheckpointFence() {
        if (failFenceWhenInactive && collectionCount == 0)
            error("collector inactive")
    ...
    fun emit(snapshot: PlaybackSessionSnapshot) {
        check(
            checkpointChannel
                .trySend(PlaybackCheckpoint.Immediate(snapshot))
                .isSuccess)
    }
}
...
private class RecordingStore(
    ...
    override suspend fun save(snapshot: PlaybackSessionSnapshot) {
        ...
        if (failSaveNumber == saveAttempts)
...
private fun snapshot(id: String, positionMillis: Long = 0L) =
    PlaybackSessionSnapshot(
        queueIds = listOf(id),
        currentTrackId = id,
...
private fun playableTracks(vararg ids: String): List<PlayableTrack> =
    ids.map { id ->
        PlayableTrack(
            id = id,
            title = id,
...
private fun libraryTracks(vararg ids: String): List<LibraryTrack> =
    ids.map { id ->
        LibraryTrack(
            id = id,
            sourceId = "source",

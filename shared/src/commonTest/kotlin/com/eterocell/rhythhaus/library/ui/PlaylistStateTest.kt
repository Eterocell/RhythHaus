...
class PlaylistStateTest {
    ...
    fun pickerStateRejectsEmptyAndBlankTrackIds() {
        assertFailsWith<IllegalArgumentException> {
            PlaylistPickerState(emptyList())
        }
        assertFailsWith<IllegalArgumentException> {
            PlaylistPickerState(listOf("track-a", " "))
        }
    }
    ...
    fun unresolvedPlaylistDetailReturnsToHubWithRecoverableMessage() {
        ...
        assertEquals(
            PlaylistDetailResolution.ReturnToHub("playlist_changed"), next)
    }
    ...
    fun confirmedSnapshotClearsLoadingAndReadFailure() {
        val snapshot =
            PlaylistSnapshot(playlists = listOf(playlist("playlist-1")))
    ...
    fun routeNoticePrioritizesRecoverableThenMutationMessage() {
        ...
        assertEquals(
            ...
            playlistRouteNotice(
                PlaylistState(mutationErrorMessage = "save_failed")),
        )
    ...
    fun tabAndBrowserSelectionArePureStateTransitions() {
        ...
        val queueSelected =
            reducePlaylistState(
                PlaylistState(),
                PlaylistStateAction.SelectTab(PlaylistTab.Queue))
        val browserOpened =
            reducePlaylistState(
                queueSelected, PlaylistStateAction.OpenBrowser(browser))

        assertEquals(PlaylistTab.Queue, browserOpened.selectedTab)
        assertEquals(browser, browserOpened.browser)
        assertTrue(
            browserOpened.browser?.selectedTrackIds?.contains("track-a") ==
                true)
    }
    ...
    fun savedPlaybackUsesEntryIdsAndExactVisibleOrderIncludingDuplicates() {
        ...
        val occurrences =
            savedPlaylistOccurrences(entries, mapOf(duplicate.id to duplicate))

        assertEquals(listOf("entry-2", "entry-1"), occurrences.map { it.id })
        assertEquals(
            listOf("track-a", "track-a"), occurrences.map { it.track.id })
    }
    ...
    fun savedRowKeysRemainIndependentForDuplicateTracks() {
        ...
        assertEquals(
            listOf("entry-1", "entry-2"), savedPlaylistRowKeys(entries))
    }
    ...
    @Test
    fun staleRefreshPublicationCannotOverwriteNewerMutationPublication() =
        runBlocking {
            val repository =
                InMemoryPlaylistRepository(
                    now = { 1L },
            ...
            assertEquals(
                listOf("playlist-1"),
                state.confirmedSnapshot.playlists.map { it.id })
            assertEquals(
                "Newer", state.confirmedSnapshot.playlists.single().name)
        }

    @Test
    fun staleRefreshFailureCannotOverwriteNewerSuccessOrClearNewerLoading() =
        runBlocking {
            var failReads = true
            val delegate =
                InMemoryPlaylistRepository(
                    now = { 1L }, idFactory = { "playlist-1" })
            val repository =
                object :
            ...
            var state =
                reducePlaylistState(
                    PlaylistState(),
                    owner.mutate(PlaylistMutationFailedMessage) {
                        create("Newer")
                    },
                )
    ...
    fun staleMutationFailureCannotAddNoticeAfterNewerSuccess() = runBlocking {
        ...
        val owner = PlaylistStateOwner(repository, Dispatchers.Default)
        val oldFailure =
            owner.mutate(PlaylistMutationFailedMessage) { create("Old") }
        failMutation = false
        var state =
            reducePlaylistState(
                PlaylistState(),

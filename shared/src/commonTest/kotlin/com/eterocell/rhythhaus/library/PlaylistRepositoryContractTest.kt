...
class PlaylistRepositoryContractTest {
    ...
    fun importPlaylistsValidatesEveryMutationBeforeCallingFactoriesOrPublishing() {
        ...
        invalidRequests.forEach { request ->
            assertFailsWith<IllegalArgumentException> { repository.importPlaylists(request) }
        }
    ...
    fun importPlaylistsPreservesRequestTrackAndDuplicateOrder() {
        ...
        val imported = repository.importPlaylists(
            listOf(
                PlaylistImportMutation(" First ", listOf("track-b", "track-a", "track-b")),
                PlaylistImportMutation("Second", listOf("track-c")),
            ),
        )

        assertEquals(listOf("playlist-1", "playlist-2"), imported.map(Playlist::id))
        assertEquals(listOf("First", "Second"), imported.map(Playlist::name))
        assertEquals(listOf("playlist-1", "playlist-2"), repository.playlists().map(Playlist::id))
        assertEquals(listOf("track-b", "track-a", "track-b"), repository.entries(imported[0].id).map(PlaylistEntry::trackId))
        assertEquals(listOf(0, 1, 2), repository.entries(imported[0].id).map(PlaylistEntry::position))
        assertEquals(3, repository.entries(imported[0].id).map(PlaylistEntry::id).toSet().size)
        assertEquals(listOf("track-c"), repository.entries(imported[1].id).map(PlaylistEntry::trackId))
    }
    ...
    fun importPlaylistsPublishesNothingWhenSecondPlaylistEntryStagingFailsAndRetryCreatesAllOnce() {
        ...
        val repository = InMemoryPlaylistRepository(
            ...
            idFactory = {
                idCall += 1
                if (failSecondPlaylistEntry && idCall == 7) error("second playlist entry id failure")
                "generated-$idCall"
            },
        )
        val existing = repository.createWithEntries("Existing", listOf("track-existing"))
        ...
        assertFailsWith<IllegalStateException> { repository.importPlaylists(request) }
        ...
        assertEquals(listOf("First", "Second"), imported.map(Playlist::name))
        assertEquals(setOf("Existing", "First", "Second"), repository.playlists().map(Playlist::name).toSet())
        assertEquals(3, repository.playlists().size)
        assertEquals(listOf("track-a", "track-a"), repository.entries(imported[0].id).map(PlaylistEntry::trackId))
        assertEquals(listOf("track-b"), repository.entries(imported[1].id).map(PlaylistEntry::trackId))
    }
}

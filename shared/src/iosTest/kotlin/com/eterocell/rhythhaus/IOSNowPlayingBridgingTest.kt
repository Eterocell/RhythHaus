...
/**
 * Tests that verify Kotlin/Native → Obj-C bridging of the nowPlayingInfo
 * dictionary.
 *
 * The `buildIOSNowPlayingDictionary` function returns a `Map<Any?, Any?>` which
 * is assigned to `MPNowPlayingInfoCenter.defaultCenter().nowPlayingInfo` (an
...
class IOSNowPlayingBridgingTest {
    ...
    fun bridging_nowPlayingInfoDoubleValuesSurviveObjCRoundTrip() {
        ...
        // Assign to nowPlayingInfo (triggers Kotlin Map → NSDictionary
        // bridging)
        ...
        assertNotNull(
            readBack, "nowPlayingInfo must be non-nil after assignment")

        // Check String values
        val title = readBack!!.get(MPMediaItemPropertyTitle)
        println(
            "[BRIDGE-TEST] title=$title type=${title?.let { it::class.simpleName }}")
        ...
        println(
            "[BRIDGE-TEST] duration=$duration type=${duration?.let { it::class.simpleName }}")
        assertNotNull(duration, "Duration must be present after round-trip")
        assertTrue(
            duration is Number,
            "Duration must be a Number after bridging — got ${duration::class.simpleName}")
        assertEquals(
            181.0,
            (duration as Number).toDouble(),
            "Duration value must survive round-trip")

        val elapsed = readBack.get(MPNowPlayingInfoPropertyElapsedPlaybackTime)
        println(
            "[BRIDGE-TEST] elapsed=$elapsed type=${elapsed?.let { it::class.simpleName }}")
        assertNotNull(elapsed, "Elapsed must be present after round-trip")
        assertTrue(
            elapsed is Number,
            "Elapsed must be a Number after bridging — got ${elapsed::class.simpleName}")
        assertEquals(
            42.0,
            (elapsed as Number).toDouble(),
            "Elapsed value must survive round-trip")

        val rate = readBack.get(MPNowPlayingInfoPropertyPlaybackRate)
        println(
            "[BRIDGE-TEST] rate=$rate type=${rate?.let { it::class.simpleName }}")
        assertNotNull(rate, "Rate must be present after round-trip")
        assertTrue(
            rate is Number,
            "Rate must be a Number after bridging — got ${rate::class.simpleName}")
        assertEquals(
            1.0,
            (rate as Number).toDouble(),
            "Rate value must survive round-trip")
        ...
        println(
            "[BRIDGE-TEST] isLiveStream=$isLive (should be null when duration is known)")
    ...
    fun bridging_nullDurationSetsIsLiveStreamAndSurvivesRoundTrip() {
        ...
        assertNull(
            readBack!!.get(MPMediaItemPropertyPlaybackDuration),
            "Duration should be absent when null")

        // IsLiveStream should be present
        val isLive = readBack.get(MPNowPlayingInfoPropertyIsLiveStream)
        println(
            "[BRIDGE-TEST] null-dur isLiveStream=$isLive type=${isLive?.let { it::class.simpleName }}")
        assertNotNull(
            isLive,
            "IsLiveStream must survive round-trip when duration is null")

        // Rate should still be present (0.0)
        val rate = readBack.get(MPNowPlayingInfoPropertyPlaybackRate)
        println(
            "[BRIDGE-TEST] null-dur rate=$rate type=${rate?.let { it::class.simpleName }}")
        assertNotNull(
            rate, "Rate must survive round-trip even with null duration")
        assertTrue(
            rate is Number,
            "Rate must be a Number — got ${rate::class.simpleName}")

...
class AboutLibrariesCatalogTest {
    ...
    @Test
    fun uiConsumedCatalogJsonParsesAndContainsDisplayableLibraries() = runBlocking {
    ...
    fun checkedInCatalogAttributesNativeTagLibDependency() = runBlocking {
        ...
        assertTrue(tagLib != null, "expected a manually-attributed taglib:taglib entry")
        assertTrue(tagLib.licenses.isNotEmpty(), "expected taglib:taglib to declare its license(s)")
        val licenseContents = tagLib.licenses.map { it.licenseContent.orEmpty() }
        assertTrue(
            licenseContents.any { it.contains("GNU LESSER GENERAL PUBLIC LICENSE") },
    ...
    fun loadAboutLibrariesReturnsLoadedWhenReadAndParseSucceed() = runBlocking {
        val state = loadAboutLibraries(readJson = { readAboutLibrariesCatalogJson() })
    ...
    @Test
    fun loadAboutLibrariesReturnsFailedWhenParsedCatalogIsEmpty() = runBlocking<Unit> {
        // com.mikepenz.aboutlibraries.Libs.Builder.build() catches its own internal parse
        // failures and yields an empty Libs rather than throwing on malformed JSON. An empty
        // attribution catalog is not a valid successful screen, so loadAboutLibraries must
    ...
    @Test
    fun loadAboutLibrariesRethrowsCancellationInsteadOfWrappingAsFailed() = runBlocking<Unit> {
        assertFailsWith<CancellationException> {
            loadAboutLibraries(readJson = { throw CancellationException("cancelled") })
        }
    }
}

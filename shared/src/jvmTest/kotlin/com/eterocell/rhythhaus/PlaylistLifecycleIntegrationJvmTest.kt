...
class PlaylistLifecycleIntegrationJvmTest {
    @Test
    fun sourceRemovalCascadesBeforePlaylistPublicationAndReconcilesResolvedQueue() = runBlocking {
        openHarness().use { harness ->
            ...
            harness.playlists.append(playlist.id, listOf("remove-track", "keep-track"))
            val savedEntries = harness.playlists.entries(playlist.id)
            harness.controller.setOccurrenceQueue(harness.occurrences(savedEntries), savedEntries.first().id)

            harness.playlists.removeEntry(savedEntries.last().id)
            assertEquals(savedEntries.map(PlaylistEntry::id), harness.controller.state.value.queue.map(QueueOccurrence::id))
            ...
            removeSourceInBackground(
                ...
                loadPlaylists = {
                    ...
                    PlaylistStateAction.SnapshotConfirmed(loadPlaylistSnapshot(harness.playlists))
                },
                platformAccess = NoOpPlatformSourceAccess,
                reconciler = harness.reconciler(events) {
                    assertEquals(emptyList(), harness.playlists.entries(playlist.id))
                },
                ioDispatcher = Dispatchers.Default,
                updateLibrary = { events += "library" },
                updatePlaylists = { action ->
                    events += "playlists"
                    publishedPlaylists = (action as PlaylistStateAction.SnapshotConfirmed).snapshot
                },
            )
            ...
            assertEquals(emptyList(), publishedPlaylists?.entriesByPlaylistId?.get(playlist.id))
            assertEquals(listOf(savedEntries.last().id), harness.controller.state.value.queue.map(QueueOccurrence::id))
            assertEquals(listOf("keep-track"), harness.controller.state.value.queue.map { it.track.id })
        }
    }

    @Test
    fun clearLibraryCascadesBeforePlaylistPublicationAndReconcilesResolvedQueue() = runBlocking {
        openHarness().use { harness ->
            ...
            harness.playlists.reorder(playlist.id, savedEntries.map(PlaylistEntry::id).reversed())
            assertEquals(savedEntries.map(PlaylistEntry::id), harness.controller.state.value.queue.map(QueueOccurrence::id))
...
private class PlaylistLifecycleHarness(
    ...
    val playlists = SqlDelightPlaylistRepository(database, now = { 1L }, idFactory = ::uuid4)
    ...
    fun occurrences(entries: List<PlaylistEntry>): List<QueueOccurrence> {
        ...
        return entries.map { entry ->
            QueueOccurrence(entry.id, requireNotNull(tracksById[entry.trackId]).toPlayableTrack())
        }
    }

    fun reconciler(events: MutableList<String>, afterCascade: () -> Unit) = PlaybackSessionReconciler { tracks ->
        events += "reconcile"
        afterCascade()
        controller.reconcileSession(tracks.map(LibraryTrack::toPlayableTrack))
...
private object NoOpPlatformSourceAccess : com.eterocell.rhythhaus.library.PlatformSourceAccess {
...
private fun openHarness(): PlaylistLifecycleHarness {
    val databaseFile: File = Files.createTempFile("rhythhaus-playlist-lifecycle", ".db").toFile().apply {
        delete()
        deleteOnExit()
    }

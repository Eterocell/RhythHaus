...
class PlaylistBackupDialogsSemanticsJvmTest {
            ...
            runComposeUiTest {
                ...
                onNode(
                        hasContentDescription(
                            "Road Mix, Missing Song, unmatched"))
                    .assertExists()
                onNode(
                        hasContentDescription(
                            "Road Mix, Duplicate Song, ambiguous"))
                    .assertExists()
            }
        }
    ...
    @OptIn(ExperimentalTestApi::class)
    @Test
    fun resultReportsSelectedCreatedSkippedAndEntryCounts() =
        withEnglishLocale {
            runComposeUiTest {
                setContent {
                    PlaylistBackupResultDialog(
                        result =
                            PlaylistBackupImportResult(
                                PlaylistImportTotals(
                                    2, 1, PlaylistImportCounts(5, 3, 4)),
                            ),
                        onDismiss = {},
                    )
                }
    ...
    @OptIn(ExperimentalTestApi::class)
    @Test
    fun maximumValidPreviewComposesOnlyViewportRowsAndCanNavigateToFinalIssue() =
        withEnglishLocale {
            runComposeUiTest {
                val reports =
                    List(1_000) { index ->
                ...
                onNode(
                        hasContentDescription(
                            "Playlist 999, Issue 99999, unmatched"))
                    .assertDoesNotExist()
                onNodeWithTag(PlaylistBackupPreviewListTag)
                    .performScrollToIndex(101_001)
                onNode(
                        hasContentDescription(
                            "Playlist 999, Issue 99999, unmatched"))
                    .assertExists()

...
internal actual fun readPlatformAudioMetadata(source: AudioSource): AudioMetadata? = when (source) {
    is AudioSource.FileDescriptor -> readAndroidMediaMetadataFromFileDescriptor(source.fd)
    is AudioSource.Uri -> readAndroidMediaMetadataFromUri(source.value)
    is AudioSource.FilePath -> null
}

private fun readAndroidMediaMetadataFromFileDescriptor(fd: Int): AudioMetadata? = readAndroidMediaMetadata {
    setDataSource("/proc/self/fd/$fd")
}

private fun readAndroidMediaMetadataFromUri(uri: String): AudioMetadata? = readAndroidMediaMetadata {
    setDataSource(LibraryDatabaseContext.applicationContext, android.net.Uri.parse(uri))
}

private fun readAndroidMediaMetadata(configure: MediaMetadataRetriever.() -> Unit): AudioMetadata? {
    ...
    return try {
        ...
        val metadata = AudioMetadata(
            ...
            discNumber = retriever.extractMetadata(MediaMetadataRetriever.METADATA_KEY_DISC_NUMBER)
                .firstPositiveNumberOrNull(),
            artworkBytes = retriever.embeddedPicture?.takeIf { it.isNotEmpty() },
        )
...
private fun AudioMetadata.hasUsefulValues(): Boolean = title != null ||
    artist != null ||
    album != null ||
    durationMillis != null ||
    trackNumber != null ||
...
private fun String?.firstPositiveNumberOrNull(): Int? = this
    ?.substringBefore('/')
    ?.trim()
    ?.toIntOrNull()
    ?.takeIf { it > 0 }

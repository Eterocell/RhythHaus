 ...
 * plus size bucket. A null size bucket is the original/full-size decode;
 * non-null buckets are thumbnails. All access is from Compose @Composable
 * functions on the main thread — no synchronization needed.
 */
object ArtworkCache {
    private val cache = HashMap<ArtworkCacheKey, ImageBitmap>(64)

    fun get(bytes: ByteArray, maxPixelSize: Int? = null): ImageBitmap? =
...
/**
 * Decodes full-size artwork with caching — first checks ArtworkCache, falls
 * back to platform decode.
 */
...
/**
 * Decodes a thumbnail-sized artwork image with caching for compact list/bar
 * surfaces.
 */
fun ByteArray.decodeArtworkThumbnailCached(

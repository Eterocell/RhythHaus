...
class InMemoryPlaylistRepository(
    ...
    override fun importPlaylists(
        ...
        val imported = validated.map { mutation ->
            ...
            mutation.trackIds.forEachIndexed { position, trackId ->
                val entry =
                    PlaylistEntry(
                        idFactory(), playlist.id, trackId, position, now())
    ...
    override fun rename(id: String, name: String) {
        val playlist =
            requireNotNull(playlists[id]) { "Playlist not found: $id" }
        playlists[id] =
            playlist.copy(
                name = requireName(name), updatedAtEpochMillis = now())
    }
    ...
    override fun append(playlistId: String, trackIds: List<String>) {
        require(playlists.containsKey(playlistId)) {
        ...
        trackIds.forEach { trackId ->
            replacement +=
                PlaylistEntry(
                    idFactory(), playlistId, trackId, replacement.size, now())
        }
    ...
    override fun removeEntry(entryId: String) {
        val entry =
            requireNotNull(entries[entryId]) {
    ...
    private fun replaceEntries(
        ...
        replacement.forEachIndexed { position, entry ->
            entries[entry.id] = entry.copy(position = position)
        }
    }
}
...
internal fun validatePlaylistImports(
    playlists: List<PlaylistImportMutation>
): List<PlaylistImportMutation> = playlists.map { mutation ->
    val name = requireName(mutation.name)
    require(mutation.trackIds.isNotEmpty()) {
        "Imported playlist track IDs must not be empty"
    }
    require(mutation.trackIds.all { it.isNotBlank() }) {
        "Imported playlist track IDs must not be blank"
    }

...
private val libraryTables = setOf("library_source", "library_track", "scan_session", "scan_error")
private val playlistTables = setOf("playlist", "playlist_entry")

private fun foreignKeyProperties(): Properties = Properties().apply { put("foreign_keys", "true") }

private fun bootstrapLegacyVersionZeroDatabase(url: String, properties: Properties) {
    val driver = JdbcSqliteDriver(url, properties)
    try {
        if (driver.userVersion() != 0L) return
        val tables = driver.userTables()
        val legacyVersion = when (tables) {
            libraryTables -> 1L
            libraryTables + playlistTables -> RhythHausDatabase.Schema.version
            else -> return
        }
...
private fun JdbcSqliteDriver.userTables(): Set<String> = executeQuery(
    identifier = null,

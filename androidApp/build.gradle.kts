...
val rhythHausVersionCode =
    providers
        .gradleProperty("rhythhaus.versionCode")
        .orElse(
            provider {
                throw GradleException(
                    "Missing required Gradle property 'rhythhaus.versionCode'")
            })
val androidAbiContract =
    extensions.getByType<RhythHausAndroidAbiContractExtension>()
val splitApkEnabled =
    shouldConfigureSplitApks(
...
android {
    ...
    defaultConfig {
        ...
        versionCode =
            rhythHausVersionCode.get().toIntOrNull()
                ?: throw GradleException(
                    "Gradle property 'rhythhaus.versionCode' must be an integer, was '${rhythHausVersionCode.get()}'")
...
val resolvedApkAnalyzer = androidSdkDirectory.map {
    resolveApkAnalyzer(it.asFile)
}
val resolvedApkSigner =
    androidSdkDirectory
        .map {
...
androidComponents.onVariants(
    androidComponents.selector().withBuildType("release")) { variant ->
        tasks.register<VerifyReleaseApksTask>("verifyReleaseApks") {
            ...
            reportFile.set(
                layout.buildDirectory.file(
                    "reports/androidReleaseVerification/release-apks.txt"))
        }
        tasks.register<VerifyReleaseAabTask>("verifyReleaseAab") {
            ...
            reportFile.set(
                layout.buildDirectory.file(
                    "reports/androidReleaseVerification/release-aab.txt"))
        }
    }

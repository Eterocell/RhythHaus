...
class PlaybackSessionSnapshotTest {
    ...
    fun sessionRoundTripKeepsDuplicateTrackOccurrencesInOrder() {
        ...
        assertEquals(
            snapshot,
            PlaybackSessionCodec.decodeSnapshot(
                PlaybackSessionCodec.encodeSnapshot(snapshot)))
    }
    ...
    fun sessionCodecRejectsDuplicateOccurrenceIdsButAllowsDuplicateTrackIds() {
        ...
        assertEquals(
            duplicateTracks,
            PlaybackSessionCodec.decodeQueue(
                PlaybackSessionCodec.encodeQueue(duplicateTracks)))
        assertFailsWith<IllegalArgumentException> {
            PlaybackSessionCodec.encodeQueue(duplicateOccurrences)
        }
    }
    ...
    fun snapshotCodecRejectsDuplicateOccurrencesAndCurrentOutsideQueue() {
        ...
        assertFailsWith<IllegalArgumentException> {
            PlaybackSessionCodec.encodeSnapshot(duplicateOccurrences)
        }
        assertFailsWith<IllegalArgumentException> {
            PlaybackSessionCodec.encodeSnapshot(missingCurrent)
        }
    }
    ...
    fun sessionQueueCodecRetainsTheExistingMaximumQueueCount() {
        val exactMaximum =
            List(PlaybackSessionCodec.maxIds) { index ->
        ...
        assertEquals(
            exactMaximum,
            PlaybackSessionCodec.decodeQueue(
                PlaybackSessionCodec.encodeQueue(exactMaximum)))
        assertFailsWith<IllegalArgumentException> {
            PlaybackSessionCodec.encodeQueue(
                exactMaximum + SessionQueueEntry("overflow", "overflow"))
        }
    }
    ...
    fun codecRoundTripsDelimitersAndEmoji() {
        ...
        assertEquals(
            ids,
            PlaybackSessionCodec.decodeIds(PlaybackSessionCodec.encodeIds(ids)))
    }

    @Test
    fun codecRejectsInvalidForms() {
        assertFailsWith<IllegalArgumentException> {
            PlaybackSessionCodec.encodeIds(listOf(""))
        }
        assertFailsWith<IllegalArgumentException> {
            PlaybackSessionCodec.encodeIds(listOf("same", "same"))
        }
        assertFailsWith<IllegalArgumentException> {
            PlaybackSessionCodec.encodeIds(listOf("\uD800"))
        }
        listOf(
                ...
                "${PlaybackSessionCodec.maxIdCharacters + 1}:${"a".repeat(PlaybackSessionCodec.maxIdCharacters + 1)}" to
                    "over-character",
                framedEncoded(
                    List(256) { index ->
                        "id-$index" +
    ...
    fun codecEnforcesBoundsAndAllOrNullDecoding() {
        ...
        assertEquals(
            ...
            PlaybackSessionCodec.decodeIds(
                PlaybackSessionCodec.encodeIds(exactMaxIds)))
        assertFailsWith<IllegalArgumentException> {
            PlaybackSessionCodec.encodeIds(exactMaxIds + "too-many")
        }

        val exactMaxCharacters =
            "x".repeat(PlaybackSessionCodec.maxIdCharacters)
        assertEquals(
            listOf(exactMaxCharacters),
            PlaybackSessionCodec.decodeIds(
                PlaybackSessionCodec.encodeIds(listOf(exactMaxCharacters))))
        assertFailsWith<IllegalArgumentException> {
        ...
        assertEquals(
            ...
            PlaybackSessionCodec.encodeIds(exactMaxEncodedBytes)
                .encodeToByteArray()
                .size,
        )
        assertFailsWith<IllegalArgumentException> {
        ...
        val largestReachableUtf8Id =
            "\u0800".repeat(PlaybackSessionCodec.maxIdCharacters)
        assertEquals(
            PlaybackSessionCodec.maxIdCharacters, largestReachableUtf8Id.length)
        assertEquals(12_288, largestReachableUtf8Id.encodeToByteArray().size)
        assertEquals(
            listOf(largestReachableUtf8Id),
            PlaybackSessionCodec.decodeIds(
                PlaybackSessionCodec.encodeIds(listOf(largestReachableUtf8Id))))
        assertFailsWith<IllegalArgumentException> {
            PlaybackSessionCodec.encodeIds(
                listOf("x".repeat(PlaybackSessionCodec.maxIdCharacters + 1)))
        }
    ...
    fun checkpointModelsExposeCompleteSnapshotsAndProgressKey() {
        ...
        assertEquals(
            snapshot,
            PlaybackCheckpoint.PlayingProgress(key, snapshot).snapshot)
    }
    ...
    private fun exactEncodedSizeIds(): List<String> {
        ...
        val remaining =
            PlaybackSessionCodec.maxEncodedUtf8Bytes -
                PlaybackSessionCodec.encodeIds(base).encodeToByteArray().size
    ...
    private fun overCountEncoded(): String =
        framedEncoded(List(PlaybackSessionCodec.maxIds + 1) { it.toString() })

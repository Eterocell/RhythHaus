...
internal fun normalizePortableText(value: String): String {
...
internal sealed interface PlaylistBackupMatch {
...
internal class PlaylistBackupMatcher(destinationTracks: List<LibraryTrack>) {
    ...
    private val candidatesByText =
        destinationTracks
            .mapNotNull { track ->
                track.durationMillis
                    ?.takeIf { it >= 0 }
                    ?.let { durationMillis ->
                        textKey(track.title, track.artist, track.album) to
                            Candidate(track.id, durationMillis / 1_000)
                    }
            }
    ...
    fun match(entry: PlaylistBackupEntry): PlaylistBackupMatch {
        val candidates =
            candidatesByText[textKey(entry.title, entry.artist, entry.album)]
                .orEmpty()
                .filter { candidate ->
                    candidate.durationSeconds in
                        (entry.durationSeconds.toLong() -
                            DURATION_TOLERANCE_SECONDS)..(entry.durationSeconds
                                .toLong() + DURATION_TOLERANCE_SECONDS)
                }
        return when (candidates.size) {
            0 -> PlaylistBackupMatch.Unmatched
            1 -> PlaylistBackupMatch.Unique(candidates.single().trackId)
            else ->
                PlaylistBackupMatch.Ambiguous(
                    candidates.map(Candidate::trackId))
        }
    }

    private fun textKey(title: String, artist: String, album: String) =
        TextKey(

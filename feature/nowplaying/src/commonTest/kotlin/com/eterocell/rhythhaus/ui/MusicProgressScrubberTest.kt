...
class MusicProgressScrubberTest {
    ...
    fun offsetMappingClampsToTrackBounds() {
        assertEquals(
            0f, scrubberFractionForOffset(offsetX = -10f, widthPx = 200f))
        assertEquals(
            0.25f, scrubberFractionForOffset(offsetX = 50f, widthPx = 200f))
        assertEquals(
            1f, scrubberFractionForOffset(offsetX = 240f, widthPx = 200f))
    ...
    fun fractionMappingClampsToDurationBounds() {
        assertEquals(
            0L,
            scrubberPositionForFraction(
                fraction = -0.25f, durationMillis = 240_000L))
        assertEquals(
            60_000L,
            scrubberPositionForFraction(
                fraction = 0.25f, durationMillis = 240_000L))
        assertEquals(
            240_000L,
            scrubberPositionForFraction(
                fraction = 1.25f, durationMillis = 240_000L))
        assertEquals(
            0L,
            scrubberPositionForFraction(fraction = 0.5f, durationMillis = 0L))
    }
    ...
    fun scrubbingPreviewIgnoresPlaybackProgressUntilRelease() {
        val state =
            MusicScrubInteractionState(
                positionMillis = 10_000L, durationMillis = 100_000L)

        state.startScrub(0.4f)
        state.updatePlaybackPosition(
            positionMillis = 12_000L, durationMillis = 100_000L)
    ...
    fun dragReleaseProducesOneFinalSeekTargetAndThenReturnsToPlaybackState() {
        ...
        state.updatePlaybackPosition(
            positionMillis = target ?: -1L, durationMillis = 100_000L)
    ...
    fun tapAndDragContractsEmitOnlyCommittedSeekTargets() {
        ...
        val state =
            MusicScrubInteractionState(
                positionMillis = 0L, durationMillis = duration)
        val emittedTargets = mutableListOf<Long>()

        state.startScrub(
            scrubberFractionForOffset(offsetX = 20f, widthPx = 200f))
        state.updateScrub(
            scrubberFractionForOffset(offsetX = 80f, widthPx = 200f))
        state.updateScrub(
            scrubberFractionForOffset(offsetX = 160f, widthPx = 200f))

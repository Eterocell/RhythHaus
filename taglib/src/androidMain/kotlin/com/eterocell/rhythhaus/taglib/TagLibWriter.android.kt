...
private class AndroidNativeTagLibWriter : TagLibWriter {
    override fun writePath(path: String, meta: WriteMeta): WriteResult = try {
        ...
        WriteResult.Unsupported(error.message ?: ANDROID_TAGLIB_NOT_PACKAGED_MESSAGE)
    } catch (error: UnsatisfiedLinkError) {
        WriteResult.Unsupported("$ANDROID_TAGLIB_NOT_PACKAGED_MESSAGE (${error.message ?: "unknown"})")
    }
}
...
private object NativeWriteLibrary {
    ...
    fun load() {
        ...
        try {
            ...
            throw NativeTagLibUnavailableException("$ANDROID_TAGLIB_NOT_PACKAGED_MESSAGE (${error.message ?: "unknown"})")
        }
    }
}
...
private const val ANDROID_TAGLIB_NOT_PACKAGED_MESSAGE = "Native TagLib writer is not available on this Android device"

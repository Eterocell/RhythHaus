...
- Legacy `queue_ids`/`current_id` reads normalize deterministically to `legacy-<index>` occurrences; coordinator restore persists the normalized new snapshot. This summary was corrected after the bounded-ID follow-up removed raw track IDs from generated occurrence IDs.

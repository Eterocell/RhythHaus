...
| P0 | Playback failure recovery | Implemented: structured fail-closed platform errors, retry, non-wrapping skip, queue-only removal, generation-safe reconciliation, and error-only localized Now Playing actions. | Manual platform smoke remains: invalidate a selected playable file on each available platform and confirm retry/skip/remove behavior; archive after recording evidence. |

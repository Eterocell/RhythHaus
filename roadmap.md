...
- [x] 除 iOS 因平台限制以外，Android 与 desktop JVM/macOS 可重复选择、重扫及移除多个独立音乐库文件夹；iOS 仍限单一 app-local source。自动化 repository/source-management/JVM/desktop/Android 验证通过；iOS simulator tests 受既有 common-test `Thread` 不兼容阻塞，仍需目标设备手动验证多文件夹与 iOS app-local 重扫流程。
